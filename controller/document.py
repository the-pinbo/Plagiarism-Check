...
class Document(BaseController):
    ...
    def post(self, *args, **kwargs):
        ...
        if content and title:
            doc = self.plag_dao.create_doc(
                content, title, description=description, author=author)
        else:
            ExceptionBuilder(BadRequest).error(
                HttpErrorCode.REQUIRED_FIELD, 'content', 'title').throw()
    ...
    def get(self):
        ...
        docs_info = dict(data=[d.to_dict()
                         for d in res['data']], count=res['count'])
        print(docs_info)
