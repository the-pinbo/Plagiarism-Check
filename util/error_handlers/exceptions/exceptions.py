...
class Error(object):
    ...
    @property
    def to_dict(self):
        self.fields = tuple(itertools.chain(*map(lambda field: field.split(config['FIELDS_SEPARATOR'])[
                            1::] if config['FIELDS_SEPARATOR'] in field else field.split(config['FIELDS_SEPARATOR']), self.fields)))
        err_dict = dict(error_constant=self.error_constant)
        if self.fields:
            err_dict['fields'] = self.fields
        if self.message:
...
class SqlAlchemyException(DatabaseException):
    ...
    def __init__(self, message=None, status_code=400, errors=()):
        ...
        super(SqlAlchemyException, self).__init__(
...
class NotFound(BaseException):
    ...
    def __init__(self, message=None, errors=()):
        ...
        super(NotFound, self).__init__(
            message=message, status_code=404, errors=errors)
...
class NotAuthenticated(BaseException):
    ...
    def __init__(self, message=None, errors=()):
        ...
        super(NotAuthenticated, self).__init__(
            message=message, status_code=401, errors=errors)
...
class Unauthrorized(BaseException):
    ...
    def __init__(self, message=None, errors=()):
        message = message or self.message
        super(Unauthrorized, self).__init__(
            message=message, status_code=403, errors=errors)
...
class ExceptionBuilder(object):
    ...
    def error(self, error_constant, *fields, message=None):
        self._errors.append(
            Error(error_constant, *fields, message=message).to_dict)
        return self
    ...
    def throw(self):
        if self._code:
            raise self._exc_cls(errors=self._errors,
                                message=self._message, status_code=self._code)
        else:
