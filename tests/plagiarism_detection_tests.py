...
class TestPlagiarismDetection(unittest.TestCase):
    ...
    def test_1_validate_doc_title(self):
        ...
        response = requests.post(
            url, headers={'Content-Type': 'application/json'}, data=json.dumps({}))
    ...
    def test_3_different_docs(self):
        ...
        self.assertEqual(
            sim_score, 0, msg='These 2 strings are completely different.')
    ...
    def test_5_similar_docs(self):
        sim_score = self.plag_detector.cosine_similarity(
            'bird parrot', 'cockatiel bird')
        self.assertGreater(
            sim_score, 0, msg='These 2 strings are similar with similarity score of {}.'.format(sim_score))
